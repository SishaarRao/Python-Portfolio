...
def check():
    ...
    while True:
        ...
        for a in neighbors:
            if not a in used or (not used[a] == None and used[a] > pos[0]):
                used[a] = pos[0]
                heappush(heap, [pos[0] - calcd(myCoords[pos[1][0]][0], myCoords[pos[1][0]][1], myCoords[word2][0], myCoords[word2][1]) + calcd(myCoords[a][0], myCoords[a][1], myCoords[word2][0], myCoords[word2][1]), [a] + pos[1]])
        heap.remove(pos)
